...
export function EditCampaignActions(
    ...
    const isEditable = status === CAMPAIGN_STATUS.DRAFT || status === CAMPAIGN_STATUS.SCHEDULED;
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            {/* Edit Button */}
            {isEditable && (
                <Button variant="secondary" className="font-bold shadow-sm" asChild disabled={isPending}>
                    <Link href={`/email/campaigns/${campaignId}/edit`}>
                        <HugeIcon name="Edit02Icon" size={16} className="mr-2"/>
                        Edit
                    </Link>
                </Button>
            )}
            ...
            {(status === CAMPAIGN_STATUS.DRAFT || status === CAMPAIGN_STATUS.SCHEDULED) && (
                <Button variant="outline" className="font-bold" onClick={handleSendNow} disabled={isPending}>
                    <HugeIcon name="SentIcon" size={16} className="mr-2"/>
                    Send Now
                </Button>
            )}
            ...
            <Button className="font-bold shadow-sm" onClick={() => setScheduleOpen(true)} disabled={isPending}>
                <HugeIcon name="Calendar04Icon" size={16} className="mr-2"/>
                ...
                <AlertDialog>
                    <AlertDialogTrigger asChild>
                        <Button variant="destructive" size="icon" disabled={isPending}>
                            <HugeIcon name="CancelCircleIcon" size={16}/>
                        </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                        <AlertDialogHeader>
