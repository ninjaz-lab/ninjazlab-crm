...
export function WalletManager({transactions}: { transactions: any[] }) {
    ...
    // Search Functions
    // Transactions
    const transactionFilterFn = (
        row: any,
        columnId: string,
        filterValue: string
    ) => {
        // 1. Safety check: If search is empty, show all rows
        if (!filterValue) return true;

        // 2. Safely convert the input to a lowercase string
        const searchValue = String(filterValue).toLowerCase();

        // 3. Check if the search matches the description
        const descriptionMatch = (row.original.description?.toLowerCase() || "").includes(searchValue);

        // 4. Check if the search matches the TRN ID
    ...
    const invoiceFilterFn = (row: any, columnId: string, filterValue: any) => {
        ...
        // 2. Check if the search matches the official Invoice Number
        const invoiceMatch = (row.original.invoiceNumber?.toLowerCase() || "").includes(searchValue);

        // 3. Check if the search matches the TRN ID
        const txId = row.original.id as string;
        const shortId = txId ? `trn-${txId.substring(0, 8).toLowerCase()}` : "";
        const idMatch = shortId.includes(searchValue);
    ...
    const PRESET_AMOUNTS = ["50", "100", "250", "500", "1000", "10000"];
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <TabsList className="bg-muted/50 p-1 rounded-xl w-fit">
                        <TabsTrigger value="transactions"
                            ...
                            <HugeIcon name="TransactionIcon" size={14} className="mr-2"/>
                            Transactions
                        </TabsTrigger>
                        <TabsTrigger value="invoices"
                                     className="font-bold rounded-lg px-6 data-[state=active]:bg-background data-[state=active]:shadow-sm">
                            <HugeIcon name="Invoice01Icon" size={14} className="mr-2"/>
                            Invoices
                        </TabsTrigger>
                    </TabsList>
                </div>
            ...
            <Dialog open={isTopUpOpen} onOpenChange={(open) => {
                ...
                <DialogContent className="sm:max-w-md p-0 overflow-hidden gap-0">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                {PRESET_AMOUNTS.map((amt) => (
                                    <Button
                                        key={amt} type="button" variant="outline"
                    ...
                    <DialogFooter className="p-6 bg-muted/20 border-t gap-3">
                        <Button variant="ghost" onClick={() => setIsTopUpOpen(false)} className="font-bold"
                                disabled={isPending}>
                            Cancel
                        </Button>
